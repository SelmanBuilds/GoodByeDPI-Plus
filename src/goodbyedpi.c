...
static struct option long_options[] = {
    ...
    {"only-programs",required_argument, 0,  'o' },
    {"debug-proc",   no_argument,       0,  'O' },
...
int main(int argc, char *argv[]) {
    ...
    int do_only_programs = 0;
    char *only_programs_file = NULL;
    int do_debug_proc = 0;
    unsigned int proc_debug_count = 0;
    ...
    while ((opt = getopt_long(argc, argv, "123456789pqrsaf:e:mwk:n", long_options, NULL)) != -1) {
        switch (opt) {
            ...
            case 't': // --fake-resend
                ...
                break;
            case 'o': // --only-programs <txtfile>
                do_only_programs = 1;
                only_programs_file = optarg;
                if (!proctrack_init(optarg)) {
                    printf("ERROR: can't load programs list '%s'\n", optarg);
                    die();
                }
                break;
            case 'O': // --debug-proc
            ...
            default:
                ...
                " --only-programs <txtfile> apply DPI bypass tricks ONLY to traffic of programs listed\n"
                "                          in the text file (one basename or path substring per line).\n"
                "                          Other programs' traffic is left untouched. # comments allowed.\n"
    ...
    if (do_only_programs) {
        printf("Per-program mode: seeding existing connections...\n");
        proctrack_seed();
        if (!proctrack_start_flow_thread(NULL)) {
            printf("ERROR: failed to start per-program FLOW tracker.\n");
            die();
        }
        proctrack_start_watcher_thread();
    }
    fflush(stdout);
    ...
    while (1) {
        if (WinDivertRecv(w_filter, packet, sizeof(packet), &packetLen, &addr)) {
            ...
            /* Per-program gate: when --only-programs is active, apply bypass
             * tricks only to traffic owned by a whitelisted process. Packets
             * from other processes are reinjected untouched and skipped.
             *
             * DNS redirect is EXEMPT from the gate: it must work globally so
             * that the system resolver (svchost.exe, not whitelisted) can still
             * resolve domains via the redirect. Without this, DNS would be
             * poisoned by the ISP and no bypass trick could help. */
            if (do_only_programs && (ppTcpHdr || ppUdpHdr) &&
                !(ppUdpHdr && (do_dnsv4_redirect || do_dnsv6_redirect)))
            {
                uint8_t  pt_proto = ppTcpHdr ? IPPROTO_TCP : IPPROTO_UDP;
                uint16_t pt_lport, pt_rport;
                const void *pt_laddr, *pt_raddr;

                if (addr.Outbound) {
                    pt_laddr = packet_v4 ? (const void *)&ppIpHdr->SrcAddr
                                         : (const void *)ppIpV6Hdr->SrcAddr;
                    pt_raddr = packet_v4 ? (const void *)&ppIpHdr->DstAddr
                                         : (const void *)ppIpV6Hdr->DstAddr;
                    pt_lport = ppTcpHdr ? ppTcpHdr->SrcPort : ppUdpHdr->SrcPort;
                    pt_rport = ppTcpHdr ? ppTcpHdr->DstPort : ppUdpHdr->DstPort;
                } else {
                    pt_laddr = packet_v4 ? (const void *)&ppIpHdr->DstAddr
                                         : (const void *)ppIpV6Hdr->DstAddr;
                    pt_raddr = packet_v4 ? (const void *)&ppIpHdr->SrcAddr
                                         : (const void *)ppIpV6Hdr->SrcAddr;
                    pt_lport = ppTcpHdr ? ppTcpHdr->DstPort : ppUdpHdr->DstPort;
                    pt_rport = ppTcpHdr ? ppTcpHdr->SrcPort : ppUdpHdr->SrcPort;
                }

                if (!proctrack_is_target(pt_proto, pt_laddr, ntohs(pt_lport),
                                         pt_raddr, ntohs(pt_rport), packet_v6)) {
                    if (do_debug_proc && proc_debug_count < 300) {
                        proc_debug_count++;
                        const char *mname = "UNKNOWN";
                        switch (proctrack_last_method) {
                            case 0: mname = "map"; break;
                            case 1: mname = "udp-loc"; break;
                            case 2: mname = "os-fallback"; break;
                        }
                        if (packet_v4) {
                            uint32_t la = *(const uint32_t *)pt_laddr;
                            uint32_t ra = *(const uint32_t *)pt_raddr;
                            printf("[GATE] %s %s %u.%u.%u.%u:%u -> %u.%u.%u.%u:%u BLOCK method=%s\n",
                                   pt_proto == IPPROTO_TCP ? "TCP" : "UDP",
                                   addr.Outbound ? "out" : "in",
                                   la & 0xFF, (la>>8)&0xFF, (la>>16)&0xFF, (la>>24)&0xFF, ntohs(pt_lport),
                                   ra & 0xFF, (ra>>8)&0xFF, (ra>>16)&0xFF, (ra>>24)&0xFF, ntohs(pt_rport),
                                   mname);
                        } else {
                            printf("[GATE] %s %s [v6]:%u -> [v6]:%u BLOCK method=%s\n",
                                   pt_proto == IPPROTO_TCP ? "TCP" : "UDP",
                                   addr.Outbound ? "out" : "in",
                                   ntohs(pt_lport), ntohs(pt_rport), mname);
                        }
                        fflush(stdout);
                    }
                    WinDivertSend(w_filter, packet, packetLen, NULL, &addr);
                    continue;
                } else if (do_debug_proc && proc_debug_count < 300) {
                    proc_debug_count++;
                    const char *mname = "UNKNOWN";
                    switch (proctrack_last_method) {
                        case 0: mname = "map"; break;
                    ...
                    if (packet_v4) {
                        uint32_t la = *(const uint32_t *)pt_laddr;
                        uint32_t ra = *(const uint32_t *)pt_raddr;
                        printf("[GATE] %s %s %u.%u.%u.%u:%u -> %u.%u.%u.%u:%u PASS method=%s\n",
                               pt_proto == IPPROTO_TCP ? "TCP" : "UDP",
                               addr.Outbound ? "out" : "in",
                               la & 0xFF, (la>>8)&0xFF, (la>>16)&0xFF, (la>>24)&0xFF, ntohs(pt_lport),
                               ra & 0xFF, (ra>>8)&0xFF, (ra>>16)&0xFF, (ra>>24)&0xFF, ntohs(pt_rport),
                        ...
                        printf("[GATE] %s %s [v6]:%u -> [v6]:%u PASS method=%s\n",
                               pt_proto == IPPROTO_TCP ? "TCP" : "UDP",
                               addr.Outbound ? "out" : "in",
                               ntohs(pt_lport), ntohs(pt_rport), mname);
                    }
